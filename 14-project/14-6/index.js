// https://jsfiddle.net/Notkabene/17za9rys/6/
...
document.addEventListener('keydown', (event) => {
  ...
  if (key === 'Tab') {
    ...
    const isThemeDark = document.body.className.includes('dark')
    if (isThemeDark) {
      document.body.style.background = '#24292E'
      tasksItem.forEach(task => {
        task.style.color = '#ffffff'
      })
      allButtons.forEach(task => {
        task.style.border = '1px solid #ffffff'
      })
